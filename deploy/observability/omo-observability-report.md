...
본 평가의 모든 실측은 **폐쇄망(air-gapped) 운영 환경**에서 수행하였다. 합성 트래픽이 아닌 실 사용 세션을 대상으로 하였으며, 업로드·반출은 차단되고 조회·다운로드만 허용되는 단방향(outbound read-only) 폐쇄망에서 사내 자체호스팅 모델과 사내/외부 조회형 시스템만으로 재현하였다(§외부 시스템 연계 참조).

| 항목 | 내용 |
|---|---|
| 네트워크 | 폐쇄망 — 업로드/반출 차단, 조회·다운로드만 허용(단방향), DS 고객사 배포 환경 |
...
**배경 — 컨텍스트 누적으로 인한 재현성 저하**: 고객사 배포 환경의 모델 컨텍스트 윈도우 한도는 약 256K 토큰(실측 설정값 기준 CodeLLM-Pro 262K, §5.5)이다. AI DLC 스킬 활용 시 ① 스킬 자체의 입력 데이터, ② AI DLC의 입력 파일(예: 요구사항정의서), ③ OMO 에이전트별 시스템 프롬프트, ④ AI DLC 단계별 산출물 작성이 함께 컨텍스트에 누적되며, 컨텍스트가 커질수록 동일 스킬을 반복 실행해도 결과물의 재현성이 떨어지는 현상이 실사용 중 관찰되었다. 원인 분석 과정에서 AI DLC 스킬 자체가 한국어로 작성되어 있음을 확인하였고, 이를 영문으로 번역했을 때 컨텍스트 크기가 얼마나 줄어드는지를 확인하기 위해 본 회귀 비교 측정을 수행하였다.
...
한국어가 영어보다 약 11% 더 많은 토큰을 소비하였다 — 한국어 텍스트가 BPE 계열 토크나이저에서 서브워드 토큰을 더 많이 사용하는 특성과 일치하는 방향이다. §5.5에서 확인한 CodeLLM-Pro 컨텍스트 사용률(선택 기간 내 최대 54.7%, 262K 한도)과 함께 보면, AI DLC 1단계 스킬 실행 한 번만으로도 한도의 상당 부분을 소비하고 있어 — 단계가 누적될수록(2단계 이후) 컨텍스트 압박과 재현성 저하 위험이 더 커질 것으로 예상된다. 나머지 AI DLC 단계에도 동일한 언어 번역 효과가 재현되는지는 후속 검증이 필요하다.

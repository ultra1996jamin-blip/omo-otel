...
[그림 2] OMO 외부 시스템 연계 — 네트워크 경계 구조 (위는 미리보기용 Mermaid, 정식 산출물은 별첨 `omo-external-systems-diagram.drawio` — [그림 1]과 동일한 색상·도형 컨벤션: 컴포넌트=서비스, 점선 라운드 박스=경계/컨테이너)

DS 사내 폐쇄망(OMO 배포 환경) 안의 OpenCode+OMO Plugin이 네트워크 경계를 거쳐 두 그룹의 외부 시스템과 연결된다. 위쪽 teal 그룹(사내 타 시스템)은 CodeLLM Max/Pro/Image·sds confluence·DS Search로, 회사 내부이지만 OMO 배포 환경 바깥의 시스템이다. 아래쪽 주황 그룹(외부 인터넷)은 grep_app(GitHub) 하나뿐이며, 인터넷을 직접 나가는 유일한 경로임을 색으로 구분했다. 가운데 빨간 점선 박스가 네트워크 경계로, "조회·다운로드는 허용, 업로드·반출은 차단"이라는 정책을 박스 라벨에 그대로 표기해 그림만으로도 정책이 읽히게 했다 — 문서(§외부 시스템 연계 본문)와 그림이 같은 사실을 이중으로 전달하는 구조다.

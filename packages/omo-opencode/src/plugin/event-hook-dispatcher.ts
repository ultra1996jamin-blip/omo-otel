...
// Tracing internal event hooks — first all ~27, then a curated subset of 7 —
// was tried twice and reverted both times. Even hooks picked as "only fires
// on a real decision" (preemptiveCompaction, backgroundNotificationHook,
// etc.) turned out to run on nearly every event tick regardless of whether
// they did anything, producing dozens of disconnected micro-traces per turn
// (observed live). The dispatcher below calls every hook unconditionally on
// every event; distinguishing "ran but no-opped" from "actually did
// something" would require instrumenting inside each hook's own
// implementation, not at this generic call site. Not worth it — see
// tool-span-tracker.ts and gen-ai-completion-span.ts for the instrumentation
// that IS working well (agent-tagged tool/LLM-call spans).
